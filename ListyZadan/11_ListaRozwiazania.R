...
# Gender - zostawiamy jako potencjalnie istotny brak
...
# zwięzłe podsumowanie WOE, IV tutaj: https://documentation.statsoft.com/STATISTICAHelp.aspx?path=WeightofEvidence/WeightofEvidenceWOEIntroductoryOverview
# sprawdzenie nieliniowych zależności

library(gam)

Formula <- as.formula("target~s(LoanAmount,2)+s(LoanAmount,2)+s(TOA,2)+s(Principal,2)+s(D_ContractDateToImportDate,2)+s(DPD,2)+ExternalAgency+Bailiff+s(Age,3)+s(LastPaymentAmount,2)+s(M_LastPaymentToImportDate,2)+s(GDPPerCapita,2)+s(MeanSalary,2)+isMale+CashLoan")
model_gam <- gam(Formula, family=binomial(link = "logit"), data = Cases)

plot.Gam(model_gam, ask=TRUE) # jak inna wersja to używamy plot.gam
...
Cases2[Age != -1, Age_d:=cut(Age,c(min(Cases2[Age >0,Age]),33,39,45,62,max(Cases2[Age >0,Age])),include.lowest = T)]
table(Cases2[,Age_d])
...
Cases <- Cases2[,.SD,.SDcols = c(Variables,"target")]
